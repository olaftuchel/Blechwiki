...
class KomponistViewModel(app: Application) : AndroidViewModel(app) {
    private val mRepository: KomponistRepository

    suspend fun getAllKomponist(query: String): LiveData<List<KomponistClass>>? {
        Log.d("KomponistViewModel", "getAllKomponist")
    ...
    suspend fun getKomponistDetails(komponistNr: Int): MutableLiveData<List<TitelInBuchClass>> {
        Log.v("KomponistViewModel", "getKomponistDetails " + komponistNr);
        return mRepository.getKomponistDetails(komponistNr);
    }

    init {
        Log.d(ContentValues.TAG, "KomponistViewModel (init)")
        mRepository = KomponistRepository(app)
    }
}
